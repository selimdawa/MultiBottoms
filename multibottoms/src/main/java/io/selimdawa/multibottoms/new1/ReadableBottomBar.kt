...
class ReadableBottomBar @JvmOverloads constructor(
    ...
    enum class ItemType(val value: Int) {
        ...
        companion object {
            fun getType(value: Int) = entries.firstOrNull { it.value == value } ?: Icon
        }
    }
    ...
    private var indicatorAnimator: ValueAnimator? = ValueAnimator.ofFloat(0f, 0f).apply {
        ...
        addUpdateListener { animation ->
            ...
            val marginParam = indicatorView?.layoutParams as? LayoutParams
    ...
    init {
        ...
        val bottomBarItemConfigList = ConfigurationXmlParser(
            context = context,
            xmlRes = tabXmlResource,
        ).parse()
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        itemWidth = if (bottomBarItemList.isNotEmpty()) layoutWidth / bottomBarItemList.size else 0f
    ...
    fun selectItem(index: Int) {
        if ((index < 0) || (index >= bottomBarItemList.size)) {
            throw IllegalArgumentException("Index should be in range of 0-${bottomBarItemList.size - 1}")
        }

        val item = bottomBarItemList[index]
        for (i in 0 until childCount) {
            val child = getChildAt(i)
            if (TAG_CONTAINER == child.tag && child is LinearLayout) {
                val selectedItemView = child.getChildAt(index) as? BottomBarItemView
                if (selectedItemView != null && selectedItemView != currentSelectedView) {
    ...
    private fun drawBottomBarItems() {
        ...
        bottomBarItemList.forEach { item ->
            val bottomBarItem = BottomBarItemView(context).apply {
                ...
                setOnClickListener {
                    if (it == currentSelectedView) return@setOnClickListener
            ...
            if (item.index == tabInitialSelectedIndex) {
                ...
                bottomBarItem.viewTreeObserver.addOnGlobalLayoutListener(object :
    ...
    private fun animateIndicator(currentItemIndex: Int) {
        ...
        val currentMargin = currentItemIndex * itemWidth
